...
Cannon::Cannon(float pXB, float pYB, float aA, float r, float g, float b) {
	...
	specialRotate = 0;
	reBuild();
	rectStand = Rectangle(STAND_LENGTH, STAND_WIDTH, -STAND_LENGTH/2, 0, -90, posXBase, posYBase, red, green, blue);
	jointBase = Joint(BASE_DIAMETER, posXBase, posYBase, red*JOINT_DARK, green*JOINT_DARK, blue*JOINT_DARK);
	rectBase = Rectangle(BASE_LENGTH, BASE_WIDTH, -BASE_LENGTH/2, 0, angleBase, posXBase, posYBase, red, green, blue);
	jointArm = Joint(ARM_DIAMETER, posXArm, posYArm, red*JOINT_DARK, green*JOINT_DARK, blue*JOINT_DARK);
	rectArm = Rectangle(ARM_LENGTH, ARM_WIDTH, -ARM_LENGTH/2, 0, angleArm, posXArm, posYArm, red, green, blue);
}
Bullet Cannon::shoot() {
	Bullet bullet;
	if (active) {
