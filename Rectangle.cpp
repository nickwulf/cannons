...
Rectangle::Rectangle(float w, float h, float rX, float rY, float ang, float pX, float pY, float r, float g, float b) {
	width = w;
	height = h;
	rotX = rX;
	rotY = rY;
...
bool Rectangle::testCollision(float bX, float bY, float rad) {
	float ptX[5];
	float ptY[5];
...
bool Rectangle::testSideClose(float p1X, float p1Y, float p2X, float p2Y, float bX, float bY, float rad) {
	bool hit = false;
	if (p1X == p2X) {
...
bool Rectangle::testSideInside(float p1X, float p1Y, float p2X, float p2Y, float testX, float testY, float cenX, float cenY) {
	bool sameSide = false;
	if (p1X == p2X) {
...
void Rectangle::setRotationPt(float rX, float rY) {
	rotX = rX;
	rotY = rY;
}
...
float Rectangle::getRotX() {
	return rotX;
}
float Rectangle::getRotY() {
	return rotY;
}
