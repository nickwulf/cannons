...
void setupGame(bool first) {
	...
	for (int i=0; i<20; i++) {
		grounds[i].setPositions(-500+50*i,tempLeft,-450+50*i,tempRight);
		tempLeft = tempRight;
		tempRight = -200 + rand()%250;
	}

	float posX = -375 + rand()%150;
	int index = ((500+posX) - (((int)posX+500)%50))/50;
	float pLX = grounds[index].getPosLeftX();
	float pLY = grounds[index].getPosLeftY();
	float pRX = grounds[index].getPosRightX();
	float pRY = grounds[index].getPosRightY();
	float slope = (pRY-pLY)/(pRX-pLX);
	float yIntercept = pLY - slope*pLX;
	float posY = slope*posX + yIntercept;
	if (first) {
		cannons[0] = Cannon(posX, posY+30, 45, 1, 0, 0);
        cannons[0].setActive(true);
		turnState = false;
		text.setText("Your Turn!");
		text.setPosition(posX-31, posY+75);
		text.reset();
	}
	else {
		cannons[0].setPosition(posX, posY+30);
		cannons[0].setAngle(45);
		cannons[0].setPower(.5);
	}

	posX = 225 + rand()%150;
	index = ((500+posX) - (((int)posX+500)%50))/50;
	pLX = grounds[index].getPosLeftX();
	pLY = grounds[index].getPosLeftY();
	pRX = grounds[index].getPosRightX();
	pRY = grounds[index].getPosRightY();
	slope = (pRY-pLY)/(pRX-pLX);
	yIntercept = pLY - slope*pLX;
	posY = slope*posX + yIntercept;
	if (first) {
		cannons[1] = Cannon(posX, posY+30, 135, 0, 0, 1);
		cannons[2] = Cannon(0, 0, 45, .5, .5, .5);
	}
	else {
		cannons[1].setPosition(posX, posY+30);
...
void displayHUD() {
	string text;
	text = "Player 1";
	drawString(-350, 270, GLUT_BITMAP_HELVETICA_18, text, 0, 0, 0);
	text = "Power: " + floatToString(cannons[0].getPower()*100, 1);
	drawString(-350, 245, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
	text = "Angle: " + floatToString(cannons[0].getAngle(), 1);
	drawString(-350, 225, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
	text = "Score: " + floatToString(cannons[0].getScore(), 1);
	drawString(-350, 200, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);

	text = "Player 2";
	drawString(270, 270, GLUT_BITMAP_HELVETICA_18, text, 0, 0, 0);
	text = "Power: " + floatToString(cannons[1].getPower()*100, 1);
	drawString(270, 245, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
	text = "Angle: " + floatToString(180 - cannons[1].getAngle(), 1);
	drawString(270, 225, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
	text = "Score: " + floatToString(cannons[1].getScore(), 1);
	drawString(270, 200, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
	...
	drawString(-350, -260, GLUT_BITMAP_HELVETICA_12, text, 0, 0, 0);
